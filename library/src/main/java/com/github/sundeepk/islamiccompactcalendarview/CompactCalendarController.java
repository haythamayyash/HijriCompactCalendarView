package com.github.sundeepk.islamiccompactcalendarview;
...
import static com.github.sundeepk.islamiccompactcalendarview.IslamicCompactCalendarView.CompactCalendarViewListener;
import static com.github.sundeepk.islamiccompactcalendarview.IslamicCompactCalendarView.FILL_LARGE_INDICATOR;
import static com.github.sundeepk.islamiccompactcalendarview.IslamicCompactCalendarView.NO_FILL_LARGE_INDICATOR;
import static com.github.sundeepk.islamiccompactcalendarview.IslamicCompactCalendarView.SMALL_INDICATOR;
...
class CompactCalendarController {
    ...
    private int firstDayOfWeekToDraw = IslamicCalendar.MONDAY;
    ...
    private IslamicCalendar currentCalender;
    private IslamicCalendar todayCalender;
    private IslamicCalendar calendarWithFirstDayOfMonth;
    private IslamicCalendar eventsCalendar;
     ...
     */
    private IslamicCalendar tempPreviousMonthCalendar;
    ...
    private void init(Context context) {
        currentCalender = new IslamicCalendar(locale);
        todayCalender = new IslamicCalendar(locale);
        calendarWithFirstDayOfMonth = new IslamicCalendar(locale);
        eventsCalendar = new IslamicCalendar(locale);
        tempPreviousMonthCalendar = new IslamicCalendar(locale);
    ...
    private void setCalenderToFirstDayOfMonth(IslamicCalendar calendarWithFirstDayOfMonth, Date currentDate, int scrollOffset, int monthOffset) {
        setMonthOffset(calendarWithFirstDayOfMonth, currentDate, scrollOffset, monthOffset);
        calendarWithFirstDayOfMonth.set(IslamicCalendar.DAY_OF_MONTH, 1);
    }

    private void setMonthOffset(IslamicCalendar calendarWithFirstDayOfMonth, Date currentDate, int scrollOffset, int monthOffset) {
        calendarWithFirstDayOfMonth.setTime(currentDate);
        calendarWithFirstDayOfMonth.add(IslamicCalendar.MONTH, scrollOffset + monthOffset);
        calendarWithFirstDayOfMonth.set(IslamicCalendar.HOUR_OF_DAY, 0);
        calendarWithFirstDayOfMonth.set(IslamicCalendar.MINUTE, 0);
        calendarWithFirstDayOfMonth.set(IslamicCalendar.SECOND, 0);
        calendarWithFirstDayOfMonth.set(IslamicCalendar.MILLISECOND, 0);
    }
    ...
    void setFirstDayOfWeek(int day) {
        if (day < 1 || day > 7) {
            throw new IllegalArgumentException("Day must be an int between 1 and 7 or DAY_OF_WEEK from Java IslamicCalendar class. For more information please see IslamicCalendar.DAY_OF_WEEK.");
        }
    ...
    void setLocale(TimeZone timeZone, ULocale locale) {
        if (locale == null) {
        ...
        this.locale = locale;
        this.timeZone = timeZone;
        this.eventsContainer = new EventsContainer(new IslamicCalendar(locale));
    ...
    void setUseWeekDayAbbreviation(boolean useThreeLetterAbbreviation) {
        ...
        this.dayColumnNames = WeekUtils.getWeekdayNames(new Locale("ar", "sa"), firstDayOfWeekToDraw, this.useThreeLetterAbbreviation);
    }
    ...
    void onMeasure(int width, int height, int paddingRight, int paddingLeft) {
        ...
        bigCircleIndicatorRadius = shouldDrawIndicatorsBelowSelectedDays && eventIndicatorStyle == IslamicCompactCalendarView.SMALL_INDICATOR ? bigCircleIndicatorRadius * 0.85f : bigCircleIndicatorRadius;
    }
    ...
    void onSingleTapUp(MotionEvent e) {
        ...
        if (dayOfMonth < calendarWithFirstDayOfMonth.getActualMaximum(IslamicCalendar.DAY_OF_MONTH)
                && dayOfMonth >= 0) {
            calendarWithFirstDayOfMonth.add(IslamicCalendar.DATE, dayOfMonth);
    ...
    private void handleHorizontalScrolling() {
        ...
        if (calendarWithFirstDayOfMonth.get(IslamicCalendar.MONTH) != currentCalender.get(IslamicCalendar.MONTH) && shouldSelectFirstDayOfMonthOnScroll) {
    ...
    int getWeekNumberForCurrentMonth() {
        IslamicCalendar calendar = new IslamicCalendar(locale);
        calendar.setTime(currentDate);
        return calendar.get(IslamicCalendar.WEEK_OF_MONTH);
    }

    Date getFirstDayOfCurrentMonth() {
        IslamicCalendar calendar = new IslamicCalendar(locale);
        calendar.setTime(currentDate);
        calendar.add(IslamicCalendar.MONTH, monthsScrolledSoFar());
    ...
    private void setToMidnight(IslamicCalendar calendar) {
        calendar.set(IslamicCalendar.HOUR_OF_DAY, 0);
        calendar.set(IslamicCalendar.MINUTE, 0);
        calendar.set(IslamicCalendar.SECOND, 0);
        calendar.set(IslamicCalendar.MILLISECOND, 0);
    }
    ...
    void drawEvents(Canvas canvas, IslamicCalendar currentMonthToDrawCalender, int offset) {
        int currentMonth = currentMonthToDrawCalender.get(IslamicCalendar.MONTH);
        List<Events> uniqEvents = eventsContainer.getEventsForMonthAndYear(currentMonth, currentMonthToDrawCalender.get(IslamicCalendar.YEAR));

        boolean shouldDrawCurrentDayCircle = currentMonth == todayCalender.get(IslamicCalendar.MONTH);
        boolean shouldDrawSelectedDayCircle = currentMonth == currentCalender.get(IslamicCalendar.MONTH);

        int todayDayOfMonth = todayCalender.get(IslamicCalendar.DAY_OF_MONTH);
        int currentYear = todayCalender.get(IslamicCalendar.YEAR);
        int selectedDayOfMonth = currentCalender.get(IslamicCalendar.DAY_OF_MONTH);
        ...
        if (uniqEvents != null) {
            for (int i = 0; i < uniqEvents.size(); i++) {
                ...
                int weekNumberForMonth = eventsCalendar.get(IslamicCalendar.WEEK_OF_MONTH);
                ...
                int dayOfMonth = eventsCalendar.get(IslamicCalendar.DAY_OF_MONTH);
                int eventYear = eventsCalendar.get(IslamicCalendar.YEAR);
    ...
    int getDayOfWeek(IslamicCalendar calendar) {
        int dayOfWeek = calendar.get(IslamicCalendar.DAY_OF_WEEK) - firstDayOfWeekToDraw;
        dayOfWeek = dayOfWeek < 0 ? 7 + dayOfWeek : dayOfWeek;
        return dayOfWeek;
    }

    void drawMonth(Canvas canvas, IslamicCalendar monthToDrawCalender, int offset) {
        ...
        boolean isSameMonthAsToday = monthToDrawCalender.get(IslamicCalendar.MONTH) == todayCalender.get(IslamicCalendar.MONTH);
        boolean isSameYearAsToday = monthToDrawCalender.get(IslamicCalendar.YEAR) == todayCalender.get(IslamicCalendar.YEAR);
        boolean isSameMonthAsCurrentCalendar = monthToDrawCalender.get(IslamicCalendar.MONTH) == currentCalender.get(IslamicCalendar.MONTH) &&
                monthToDrawCalender.get(IslamicCalendar.YEAR) == currentCalender.get(IslamicCalendar.YEAR);
        int todayDayOfMonth = todayCalender.get(IslamicCalendar.DAY_OF_MONTH);
        boolean isAnimatingWithExpose = animationStatus == EXPOSE_CALENDAR_ANIMATION;

        int maximumMonthDay = monthToDrawCalender.getActualMaximum(IslamicCalendar.DAY_OF_MONTH);

        tempPreviousMonthCalendar.setTimeInMillis(monthToDrawCalender.getTimeInMillis());
        tempPreviousMonthCalendar.add(IslamicCalendar.MONTH, -1);
        int maximumPreviousMonthDay = tempPreviousMonthCalendar.getActualMaximum(IslamicCalendar.DAY_OF_MONTH);
        ...
        for (int dayColumn = 0, colDirection = isRtl ? 6 : 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                if (currentCalender.get(IslamicCalendar.DAY_OF_MONTH) == day && isSameMonthAsCurrentCalendar && !isAnimatingWithExpose) {
