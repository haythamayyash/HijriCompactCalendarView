...
import com.github.sundeepk.islamiccompactcalendarview.IslamicCompactCalendarView;
import com.github.sundeepk.islamiccompactcalendarview.IslamicCompactCalendarView.CompactCalendarAnimationListener;
...
public class ApplicationTest {
    ...
    private IslamicCompactCalendarView islamicCompactCalendarView;
    ...
    public void setUp() throws Exception {
        ...
        islamicCompactCalendarView = (IslamicCompactCalendarView) activityRule.getActivity().findViewById(R.id.compactcalendar_view);
    ...
    public void testItDrawsEventsRtl(){
        ...
        islamicCompactCalendarView.setIsRtl(true);
        ...
        assertEquals(getEventsFor(Calendar.MARCH, 2015), islamicCompactCalendarView.getEventsForMonth(currentCalender.getTime()));
    ...
    public void testOnDayClickListenerIsCalledWhenLocaleIsFranceWithWedAsFirstDayOFWeek(){
        ...
        islamicCompactCalendarView.setListener(listener);
    ...
    public void testOpenedAndClosedListerCalledForExposeAnimationCalendar() throws Throwable {
        ...
        islamicCompactCalendarView.setAnimationListener(listener);
    ...
    private void waitForAnimationFinish() throws Exception {
        ConditionWatcher.waitForCondition(new Instruction() {
            ...
            public boolean checkCondition() {
                return !islamicCompactCalendarView.isAnimating();
            }
        });
    }
    ...
    public void testItDoesNotThrowNullPointerWhenNoAnimationListenerIsSet() throws Throwable {
        //Sun, 08 Feb 2015 00:00:00 GMT
        islamicCompactCalendarView.setAnimationListener(null);
        setDate(new Date(1423353600000L));
        onView(withId(R.id.show_with_animation_calendar)).perform(click());
        onView(withId(R.id.slide_calendar)).perform(click());
    }
    ...
    public void testItDrawsDifferentColorsForCurrentSelectedDay(){
        //Sun, 08 Feb 2015 00:00:00 GMT
        setDate(new Date(1423353600000L));
        islamicCompactCalendarView.setCurrentDayTextColor(Color.BLACK);
        islamicCompactCalendarView.setCurrentSelectedDayTextColor(Color.BLUE);
        takeScreenShot();
    }

    @Test
    public void testWhenShouldSelectFirstDayOfMonthOnScrollIsFalseItDoesNotSelectFIrstDayOfMonth()  {
        islamicCompactCalendarView.shouldSelectFirstDayOfMonthOnScroll(false);
        setDate(new Date(1423353600000L));
        scrollCalendarForwardBy(1);
        takeScreenShot();
    }
    ...
    private void syncToolbarDate(){
        getInstrumentation().runOnMainSync(new Runnable() {
            ...
            public void run() {
                ...
                toolbar.setTitle(dateFormatForMonth.format(islamicCompactCalendarView.getFirstDayOfCurrentMonth()));
            }
        });
    }
    ...
    private void setFirstDayOfWeek(final int dayOfWeek) {
        getInstrumentation().runOnMainSync(new Runnable() {
            @Override
            public void run() {
                islamicCompactCalendarView.setFirstDayOfWeek(dayOfWeek);
            }
        });
    }
    ...
    private void setUseThreeLetterAbbreviation(final boolean useThreeLetterAbbreviation) {
        getInstrumentation().runOnMainSync(new Runnable() {
            @Override
            public void run() {
                islamicCompactCalendarView.setUseThreeLetterAbbreviation(useThreeLetterAbbreviation);
            }
        });
    }
    ...
    private void setShouldDrawDaysFromOtherMonths(final boolean shouldDrawEventsBelowDayIndicators) {
        getInstrumentation().runOnMainSync(new Runnable() {
            @Override
            public void run() {
                islamicCompactCalendarView.displayOtherMonthDays(shouldDrawEventsBelowDayIndicators);
            }
        });
    }
    ...
    private void setDrawEventsBelowDayIndicators(final boolean shouldDrawEventsBelowDayIndicators) {
        getInstrumentation().runOnMainSync(new Runnable() {
            @Override
            public void run() {
                islamicCompactCalendarView.shouldDrawIndicatorsBelowSelectedDays(shouldDrawEventsBelowDayIndicators);
            }
        });
    }
    ...
    private void setIndicatorType(final int currentSelectedDayStyle, final int eventStyle, final int currentDayStyle) {
        getInstrumentation().runOnMainSync(new Runnable() {
            @Override
            public void run() {
                islamicCompactCalendarView.setCurrentSelectedDayIndicatorStyle(currentSelectedDayStyle);
                islamicCompactCalendarView.setEventIndicatorStyle(eventStyle);
                islamicCompactCalendarView.setCurrentDayIndicatorStyle(currentDayStyle);
            }
        });
    }
    ...
    private void capture(final String name) {
        getInstrumentation().runOnMainSync(new Runnable() {
            @Override
            public void run() {
                islamicCompactCalendarView.requestLayout();
    ...
    private void setDate(final Date date) {
        getInstrumentation().runOnMainSync(new Runnable() {
            @Override
            public void run() {
                islamicCompactCalendarView.setCurrentDate(date);
                ActionBar toolbar = activityRule.getActivity().getSupportActionBar();
                toolbar.setTitle(dateFormatForMonth.format(islamicCompactCalendarView.getFirstDayOfCurrentMonth()));
            }
        });
    ...
    private void shouldSelectFirstDayOfMonthOnScroll(final boolean shouldSelectFirstDay) {
        getInstrumentation().runOnMainSync(new Runnable() {
            @Override
            public void run() {
                islamicCompactCalendarView.shouldSelectFirstDayOfMonthOnScroll(shouldSelectFirstDay);
                ActionBar toolbar = activityRule.getActivity().getSupportActionBar();
                toolbar.setTitle(dateFormatForMonth.format(islamicCompactCalendarView.getFirstDayOfCurrentMonth()));
            }
        });
    ...
    private void addEvents(final int month, final int year) {
        Context context = islamicCompactCalendarView.getContext();
        ((Activity) context).runOnUiThread(new Runnable() {
            @Override
            public void run() {
                islamicCompactCalendarView.addEvents(getEventsFor(month, year));
            }
        });
    }
    ...
    public void setHeight(final float height) {
        final Context context = islamicCompactCalendarView.getContext();
        ((Activity) context).runOnUiThread(new Runnable() {
            @Override
            public void run() {
                int newHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, height, context.getResources().getDisplayMetrics());
                islamicCompactCalendarView.getLayoutParams().height = newHeight;
                islamicCompactCalendarView.setTargetHeight(newHeight);
                islamicCompactCalendarView.requestLayout();
                islamicCompactCalendarView.invalidate();
            }
        });
    }
